...
{
    const ICollideable* pCollide = (gamehelpers->EdictOfIndex(gamehelpers->EntityToBCompatRef(const_cast<CBaseEntity*>(pEntity))))->GetCollideable();
    NDebugOverlay::BoxAngles(pCollide->GetCollisionOrigin(), pCollide->OBBMins(), pCollide->OBBMaxs(), pCollide->GetCollisionAngles(), r, g, b, a, flDuration);
}
...
void NDebugOverlay::DrawTickMarkedLine(const Vector& startPos, const Vector& endPos, float tickDist, int tickTextDist, int r, int g, int b, bool noDepthTest, float duration)
{
    ...
    vBodyDir = vec3_origin;//pPlayer->BodyDirection2D();
